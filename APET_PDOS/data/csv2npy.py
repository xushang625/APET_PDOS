...
def csv2npy(df_all, pwd, split, stratify=None, random_states=0):
    ...
    # 保存索引为独立的文件
    np.save(os.path.join(pwd, "train", "%s_apet_train_index.npy" % split), train_df.index.to_numpy())
    np.save(os.path.join(pwd, "valid", "%s_apet_valid_index.npy" % split), valid_df.index.to_numpy())
    np.save(os.path.join(pwd, "test", "%s_apet_test_index.npy" % split), test_df.index.to_numpy())
